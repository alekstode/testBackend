...
namespace BLL.Local
{
    public partial class LocalProvider : IComplexProvider
    {
        ...
        {
            if (typeof(SexDto) == dtoType) { LocalSexService = new LocalSexService(uow); AllServices.Add(typeof(SexDto), LocalSexService); }
            if (typeof(StudentDto) == dtoType) { LocalStudentService = new LocalStudentService(uow); AllServices.Add(typeof(StudentDto), LocalStudentService); }
            if (typeof(AcademicPerformanceDto) == dtoType) { LocalAcademicPerformanceService = new LocalAcademicPerformanceService(uow); AllServices.Add(typeof(AcademicPerformanceDto), LocalAcademicPerformanceService); }
        }
